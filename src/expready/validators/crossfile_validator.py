...
def validate_metadata_vs_matrix(metadata_table: Table, matrix_table: Table) -> list[Issue]:
    issues: list[Issue] = []
    if "sample_id" not in metadata_table.columns or not matrix_table.columns:
        return issues

    matrix_columns = infer_sample_columns(matrix_table)
    metadata_ids = set(metadata_table.column_values("sample_id"))
...
def validate_metadata_vs_manifest(
    ...
    *,
    ...
    issues: list[Issue] = []
    if "sample_id" not in metadata_table.columns:
        return issues
